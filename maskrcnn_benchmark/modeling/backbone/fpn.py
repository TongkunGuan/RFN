...
class FPNCAT(nn.Module):
    ...
    def forward(self, x):
        ...
        # unpooling first
        # last_inner = F.interpolate(x[-1], size=x[-2].shape[-2:], mode='bilinear', align_corners=False)

        for feature, inner_block, layer_block in zip(
            x[:-1][::-1], self.inner_blocks[:-1][::-1], self.layer_blocks[:-1][::-1]
        ):
            last_inner = F.interpolate(last_inner, size=feature.shape[-2:], mode='bilinear', align_corners=False)
            inner_lateral = torch.cat([last_inner, feature], dim=1)

            last_inner = getattr(self, inner_block)(inner_lateral)
            results.insert(0, getattr(self, layer_block)(last_inner))
