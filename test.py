...
from utils.utils import check_and_validate_polys, generate_global_input_images_mask,box_iou_xyxy
...
parser.add_argument('--model_path', default='weights/ICDAR2017MLT.pth', type=str,help='resume from checkpoint')
parser.add_argument('--dataset', default='ICDAR2017MLT', type=str, help='select testing dataset')
...
if test:
    ...
    for index in range(math.ceil(len(val_list)//args.batchsize)):
        ...
        for single_index in range(args.batchsize):
            ...
            refine_quad_bboxes, refine_quad_scores = encoder.refine_score(recur_proposals, args.cls_thresh,args.nms_thresh,gts_preds, cfg.MODEL.RRPN.GT_BOX_MARGIN,args.input_size)
            ...
            for quad in quad_boxes:
                if args.dataset in ["MSC2020_build",'MSRA-TD500','ICDAR2017MLT']:
            ...
            if save_img:
                ...
                # draw gt points
                gt_anno = open(path+"/%s/ICDAR2017MLT_validation_GT/gt_%s.txt" % (args.dataset,_img[:-4]), "r")
                gt_anno = gt_anno.readlines()
                """
                you need modify some codes for different dataset!
                """
                if args.dataset in ["MSC2020_build",'MSRA-TD500','ICDAR2017MLT','USTB-SV1K']:
...
if eval:
    ...
    if args.dataset=='MPSC':
        gt_path="./data/MPSC/GT_MSC_Test.zip"
    ...
    if args.dataset in ['MPSC','INDAR2017-MLT','ICDAR2013']:
        query0 = "python ./eval/%s/script.py -g=%s -s=%s" % (path1, args.dataset, gt_path, eval_dir + args.output_zip)
        ...
        print("test_hmean for cls_threshold: {:.4f}  precise:{:.4f}   recall:{:.4f}".format(hmean0,precise0, recall0))
