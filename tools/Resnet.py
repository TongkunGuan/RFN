...
class ResNetModels(object):
    ...
    def __init__(self):
        self.path = './resnet50-19c8e357.pth'
    def resnet50(self, **kwargs):
        """Constructs a ResNet-50 model.
        Args:
            pretrained (bool): If True, returns a model pre-trained on Places
        """
        model = ResNet(Bottleneck, [3, 4, 6, 3], deep_base=False,bn_type='inplace_abn', **kwargs)
        model = ModuleHelper.load_model(model, pretrained=self.path)
        return model
    ...
    def deepbase_resnet50(self, **kwargs):
        ...
        Args:
            pretrained (bool): If True, returns a model pre-trained on Places
        """
        model = ResNet(Bottleneck, [3, 4, 6, 3], deep_base=True,bn_type='inplace_abn', **kwargs)
        model = ModuleHelper.load_model(model, pretrained=self.path)
        return model
...
def test():
    multi_grid = [1, 1, 1]
    orig_resnet = ResNetModels().resnet50()
    arch_net = DilatedResnetBackbone(orig_resnet, dilate_scale=16, multi_grid=multi_grid)
test()